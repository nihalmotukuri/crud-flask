...
def add_author():
    ...
    if request.method == 'POST':
        ...
        return redirect()
    return render_template('add_author.html', authors_list = authors_list)
...
def add_book():
    ...
    if request.method == 'POST':
        ...
        return redirect()
    return render_template('add_book.html', books_list = books_list)

@app.route('/update_book/<id>', methods=['POST'])
def update_book(id):
    book = books.find({ '_id': ObjectId(id) })
    authors = request.form.getlist('authors')
    books.update_one({'_id': id}, {'$set': {'title': book.title, authors}})
