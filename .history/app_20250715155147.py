...
@app.route('/get_books')
def get_books():
    books_dict = books.find({})
    books_list = list(books_dict)
    author_names = []
    for a_id in books_dict.get('author_ids'):
        author = authors.find({'_id': ObjectId(a_id)})
        author_names.append(author['author_name'])
    return render_template('books.html', books_list = books_list, author_names = author_names)

@app.route('/get_authors')
def get_authors():
    authors_list = list(authors.find({}))
    book_titles = []
    for b_id in authors_list.get('books_ids'):
        book = books.find({'_id': ObjectId(b_id)})
        book_titles.append(books['author_name'])
    return render_template('authors.html', authors_list = authors_list, book_titles = book_titles)
...
def update_book(book_id):
    ...
    if request.method == 'POST':
        ...
        for a_id in author_ids:
            author = authors.find_one({'_id': ObjectId(a_id)})
            if author.get('book_ids'):
        ...
        books.update_one({'_id': ObjectId(book_id)}, {'$set': {'author_ids': author_ids}})
        return redirect('/get_books')
...
def update_author(author_id):
    ...
    if request.method == 'POST':
        ...
        return redirect('/get_authors')
    return render_template('update_author.html', book_titles = book_titles)
