...
def update_book(book_id):
    ...
    if request.method == 'POST':
        ...
        author_dict = authors.find_one({'_id': ObjectId(book_id)})
        author_ids = book.get('author_ids', [])
        req_ids = request.form.getlist('authors')
        for r_id in req_ids:
            if r_id not in author_ids:
                author_ids.append(r_id)
        author_names = []
        book_ids = book.get('book_ids', [])
