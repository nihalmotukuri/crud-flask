...
def update_book(book_id):
    ...
    if request.method == 'POST':
        ...
        books.update_one({'_id': ObjectId(book_id)}, {'$set': {'title': title, 'author_ids': author_ids, 'author_names': author_names}})
        return redirect('/get_books')
    return render_template('update_book.html', authors_list = authors_list)

@app.route('/update_author/<author_id>', methods=['GET', 'POST'])
def update_author(author_id):
    author= books.find_one({ '_id': ObjectId(author_id) })
    books_list = list(books.find({}))
    if request.method == 'POST':
        name = author['name']
        ...
        authors.update_one({'_id': ObjectId(author_id)}, {'$set': {'author_name': name, 'book_ids': book_ids, 'book_titles': book_titles}})
        return redirect('/get_authors')
    return render_template('update_author.html', authors_list = authors_list)
