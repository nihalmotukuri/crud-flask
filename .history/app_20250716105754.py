...
def get_books():
    ...
    return render_template('books.html', books_list = books_list, authors_list = authors_list)

@app.route('/get_authors')
def get_authors():
    books_list = list(books.find({}))
    authors_list = list(authors.find({}))
    return render_template('authors.html', books_list = books_list, authors_list = authors_list)
    ...
    if request.method == 'POST':
        ...
        for author in authors_list:
            for a_id in author_ids:
                if ObjectId(a_id) == author['_id']:
                    author_names.append(author['author_name'])
                    selected_author = authors.find_one({'_id': ObjectId(a_id)})
                    book_ids = selected_author.get('book_ids', [])
                    book_titles = selected_author.get('book_titles', [])
                    
                    if ObjectId(book_id) not in book_ids:
                        book_ids.append(book_id)
                        current_book = books.find_one({'_id': ObjectId(book_id)})
                        book_titles.append(current_book['title'])
                    
                    book_ids = list(set(book_ids))
                    book_titles = list(set(book_titles))

                    authors.update_one({'_id': ObjectId(a_id)}, {'$set': {'book_titles': book_titles, 'book_ids': book_ids}})
    ...
    if request.method == 'POST':
        ...
        for book in books_list:
            for b_id in book_ids:
                if ObjectId(b_id) == book['_id']:
                    book_titles.append(book['author_name'])
                    selected_book = books.find_one({'_id': ObjectId(b_id)})
                    author_ids = selected_book.get('author_ids', [])
                    author_names = selected_book.get('author_names', [])
                    
                    if ObjectId(author_id) not in author_ids:
                        author_ids.append(author_id)
                        current_author = authors.find_one({'_id': ObjectId(author_id)})
                        author_names.append(current_author['author_name'])
                    
                    author_ids = list(set(author_ids))
                    authors.update_one({'_id': ObjectId(b_id)}, {'$set': {'author_ids': author_ids}})
                    author_names = list(set(author_names))

                    books.update_one({'_id': ObjectId(b_id)}, {'$set': {'author_names': author_names, 'author_ids': author_ids}})

        authors.update_one({'_id': ObjectId(author_id)}, {'$set': {'book_ids': book_ids, 'book_titles': book_titles}})

        return redirect('/get_authors')
    
    return render_template('update_author.html' books_list = books_list)
